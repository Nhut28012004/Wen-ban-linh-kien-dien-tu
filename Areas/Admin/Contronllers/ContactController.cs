    ...
    public class ContactController : Controller
    {
        ...
        public async Task<IActionResult> Edit(ContactModel contact)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrWhiteSpace(contact.Map) &&
                    (contact.Map.StartsWith("https://maps.app.goo.gl", StringComparison.OrdinalIgnoreCase)
                     || contact.Map.Contains("goo.gl/maps", StringComparison.OrdinalIgnoreCase)))
                {
                    try
                    {
                        using var httpClient = new HttpClient(new HttpClientHandler { AllowAutoRedirect = true });
                        httpClient.Timeout = TimeSpan.FromSeconds(8);
                        var response = await httpClient.GetAsync(contact.Map);
                        var finalUri = response.RequestMessage?.RequestUri?.ToString();
                        if (!string.IsNullOrWhiteSpace(finalUri))
                        {
                            contact.Map = finalUri;
                        }
                    }
                    catch
                    {
